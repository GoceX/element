      ...
      <div class="container">
        ...
        <ul class="nav">
          ...
          <li class="nav-item nav-item-theme">
            <router-link active-class="active" :to="`/${lang}/theme`">{{ langConfig.theme }}
            </router-link>
          </li>
          <!-- 资源 -->
          <li class="nav-item">
          ...
          <li class="nav-item nav-versions" v-show="isComponentPage">
            <span class="is-active">版本：{{ version }}</span>
          </li>
...
import testImg from '../assets/images/web.png';
...
export default {
  ...
  mounted() {
    ...
    testInnerImg.onload = () => {
      this.$isEle = true;
      if (typeof ga === 'function') ga('send', 'event', 'DocView', 'Ali', 'Inner');
    };
    // testInnerImg.onerror = (err) => {
    //   if (typeof ga === 'function') ga('send', 'event', 'DocView', 'Ali', 'Outer');
    //   // 使用本地资源避免外网加载失败的报错
    //   testInnerImg.src = testImg;
    // };
    testInnerImg.src = testImg;
  },
  methods: {
